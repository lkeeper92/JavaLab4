...
public class GraphicsDisplay extends JPanel {
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        for (Double[] point: graphicsData) {
            ...
            if (Math.abs((int)Math.round(point[1])) < 10) {
                // Вычисляем сумму цифр с использованием различных методов
                int sum = 0;
                int number = Math.abs((int)Math.round(point[1]));
                while (number != 0) {
                    sum += number % 10;
                    number /= 10;
                }
                if (sum<10) {
                    canvas.setColor(Color.BLUE); // Установить цвет для контуров маркеров
                    canvas.setPaint(Color.BLUE);
                } else {
                    canvas.setColor(Color.BLACK); // Установить цвет для контуров маркеров
                    canvas.setPaint(Color.BLACK);
                }
// Установить цвет для закрашивания маркеров внутри
                canvas.setColor(Color.BLUE); // Установить цвет для контуров маркеров
                canvas.setPaint(Color.BLUE);
            }
            else{
